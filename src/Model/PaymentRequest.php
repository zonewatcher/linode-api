...
class PaymentRequest implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    public function listInvalidProperties()
    {
        ...
        if (!preg_match("/^\\$?\\d+\\.\\d{2}$/", $this->container['usd'])) {
            $invalidProperties[] = "invalid value for 'usd', must be conform to the pattern /^\\$?\\d+\\.\\d{2}$/.";
        }
     ...
     * Sets cvv
     *
     * @param string|null $cvv CVV (Card Verification Value) of the credit card to be used for the Payment. Required if paying by credit card.
     *
     * @return self
     */
     ...
     * Sets usd
     *
     * @param string $usd The amount in US Dollars of the Payment.  * Can begin with or without `$`. * Commas (`,`) are not accepted. * Must end with a decimal expression, such as `.00` or `.99`. * Minimum: `$5.00` or the Account balance, whichever is lower. * Maximum: `$2000.00` or the Account balance up to `$50000.00`, whichever is greater.
     *
     * @return self
     */
    public function setUsd($usd)
    {
        ...
        if ((!preg_match("/^\\$?\\d+\\.\\d{2}$/", $usd))) {
            throw new \InvalidArgumentException("invalid value for $usd when calling PaymentRequest., must conform to the pattern /^\\$?\\d+\\.\\d{2}$/.");
        }
     ...
     *
     * @param int|null $payment_method_id The ID of the Payment Method to apply to the Payment.
     *
     * @return self
     */
