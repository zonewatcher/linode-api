...
/**
 * LinodeConfigInterface Class Doc Comment
 *
 * @category Class
 ...
 */
class LinodeConfigInterface implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    /**
      ...
      * @var string
      */
    protected static $openAPIModelName = 'LinodeConfigInterface';
    ...
    const PURPOSE__PUBLIC = 'public';
    const PURPOSE_VLAN = 'vlan';
    ...
    public function __construct(array $data = null)
    {
        ...
        $this->container['ipam_address'] = $data['ipam_address'] ?? null;
    ...
    public function listInvalidProperties()
    {
        ...
        if (!is_null($this->container['label']) && !preg_match("/[a-z0-9-]+/", $this->container['label'])) {
            $invalidProperties[] = "invalid value for 'label', must be conform to the pattern /[a-z0-9-]+/.";
        }
     ...
     * Sets label
     *
     * @param string|null $label The name of this interface.  Required for `vlan` purpose interfaces. Must be an empty string or `null` for `public` purpose interfaces.  If the VLAN label is new, a VLAN is created. Up to 10 VLANs can be created in each data center region. To view your active VLANs, use the [VLANs List](/docs/api/networking/#vlans-list) endpoint.  May only consist of ASCII letters, numbers, and dashes (`-`).  Must be unique among the Linode's interfaces.
     *
     * @return self
     */
    public function setLabel($label)
    {
        if (!is_null($label) && (mb_strlen($label) > 64)) {
            throw new \InvalidArgumentException('invalid length for $label when calling LinodeConfigInterface., must be smaller than or equal to 64.');
        }
        if (!is_null($label) && (mb_strlen($label) < 1)) {
            throw new \InvalidArgumentException('invalid length for $label when calling LinodeConfigInterface., must be bigger than or equal to 1.');
        }
        if (!is_null($label) && (!preg_match("/[a-z0-9-]+/", $label))) {
            throw new \InvalidArgumentException("invalid value for $label when calling LinodeConfigInterface., must conform to the pattern /[a-z0-9-]+/.");
        }
     ...
     * @return string|null
     */
    public function getIpamAddress()
    {
        return $this->container['ipam_address'];
    }

    /**
     * Sets ipam_address
     *
     * @param string|null $ipam_address This Network Interface's private IP address in Classless Inter-Domain Routing (CIDR) notation.  Only used for `vlan` purpose interfaces. Must be an empty string or `null` for `public` purpose interfaces.  The Linode is configured to use this address for the associated interface upon reboot if Network Helper is enabled. If Network Helper is disabled, the address can be enabled with [manual static IP configuration](/docs/guides/linux-static-ip-configuration/).  Must be unique among the Linode's interfaces.
     *
     * @return self
     */
    public function setIpamAddress($ipam_address)
    {
        $this->container['ipam_address'] = $ipam_address;
