...
/**
 * SSHKeyRequest Class Doc Comment
 *
 * @category Class
 ...
 */
class SSHKeyRequest implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    /**
      ...
      * @var string
      */
    protected static $openAPIModelName = 'SSHKeyRequest';
     ...
     * Sets label
     *
     * @param string|null $label A label for the key.
     *
     * @return self
     */
    public function setLabel($label)
    {
        if (!is_null($label) && (mb_strlen($label) > 64)) {
            throw new \InvalidArgumentException('invalid length for $label when calling SSHKeyRequest., must be smaller than or equal to 64.');
        }
        if (!is_null($label) && (mb_strlen($label) < 1)) {
            throw new \InvalidArgumentException('invalid length for $label when calling SSHKeyRequest., must be bigger than or equal to 1.');
        }
     ...
     * Sets ssh_key
     *
     * @param string|null $ssh_key The public SSH Key, which is used to authenticate to the root user of the Linodes you deploy.
     *
     * @return self
     */
