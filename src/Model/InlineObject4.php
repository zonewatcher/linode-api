...
class InlineObject4 implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    public function listInvalidProperties()
    {
        ...
        if (!preg_match("/\\A(\\*\\.)?([a-zA-Z0-9-_]{1,63}\\.)+([a-zA-Z]{2,3}\\.)?([a-zA-Z]{2,16}|xn--[a-zA-Z0-9]+)\\Z/", $this->container['domain'])) {
     ...
     * Sets domain
     *
     * @param string $domain The new domain for the clone. Domain labels cannot be longer than 63 characters and must conform to [RFC1035](https://tools.ietf.org/html/rfc1035). Domains must be unique on Linode's platform, including across different Linode accounts; there cannot be two Domains representing the same domain.
     *
     * @return self
     */
    public function setDomain($domain)
    {
        if ((mb_strlen($domain) > 255)) {
            throw new \InvalidArgumentException('invalid length for $domain when calling InlineObject4., must be smaller than or equal to 255.');
        }
        if ((mb_strlen($domain) < 1)) {
            throw new \InvalidArgumentException('invalid length for $domain when calling InlineObject4., must be bigger than or equal to 1.');
        }
        if ((!preg_match("/\\A(\\*\\.)?([a-zA-Z0-9-_]{1,63}\\.)+([a-zA-Z]{2,3}\\.)?([a-zA-Z]{2,16}|xn--[a-zA-Z0-9]+)\\Z/", $domain))) {
            throw new \InvalidArgumentException("invalid value for $domain when calling InlineObject4., must conform to the pattern /\\A(\\*\\.)?([a-zA-Z0-9-_]{1,63}\\.)+([a-zA-Z]{2,3}\\.)?([a-zA-Z]{2,16}|xn--[a-zA-Z0-9]+)\\Z/.");
        }
