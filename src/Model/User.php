...
class User implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    public function __construct(array $data = null)
    {
        ...
        $this->container['tfa_enabled'] = $data['tfa_enabled'] ?? null;
    }
    ...
    public function listInvalidProperties()
    {
        ...
        if (!is_null($this->container['username']) && (mb_strlen($this->container['username']) < 3)) {
            $invalidProperties[] = "invalid value for 'username', the character length must be bigger than or equal to 3.";
        }

        if (!is_null($this->container['username']) && !preg_match("/^[a-zA-Z0-9]((?![_-]{2,})[a-zA-Z0-9-_])+[a-zA-Z0-9]$/", $this->container['username'])) {
            $invalidProperties[] = "invalid value for 'username', must be conform to the pattern /^[a-zA-Z0-9]((?![_-]{2,})[a-zA-Z0-9-_])+[a-zA-Z0-9]$/.";
        }
     ...
     * Sets username
     *
     * @param string|null $username The User's username. This is used for logging in, and may also be displayed alongside actions the User performs (for example, in Events or public StackScripts).
     *
     * @return self
     */
    public function setUsername($username)
    {
        if (!is_null($username) && (mb_strlen($username) > 32)) {
            throw new \InvalidArgumentException('invalid length for $username when calling User., must be smaller than or equal to 32.');
        }
        if (!is_null($username) && (mb_strlen($username) < 3)) {
            throw new \InvalidArgumentException('invalid length for $username when calling User., must be bigger than or equal to 3.');
        }
        if (!is_null($username) && (!preg_match("/^[a-zA-Z0-9]((?![_-]{2,})[a-zA-Z0-9-_])+[a-zA-Z0-9]$/", $username))) {
            throw new \InvalidArgumentException("invalid value for $username when calling User., must conform to the pattern /^[a-zA-Z0-9]((?![_-]{2,})[a-zA-Z0-9-_])+[a-zA-Z0-9]$/.");
        }
     ...
     * Sets email
     *
     * @param string|null $email The email address for the User. Linode sends emails to this address for account management communications. May be used for other communications as configured.
     *
     * @return self
     */
     ...
     * Sets restricted
     *
     * @param bool|null $restricted If true, the User must be granted access to perform actions or access entities on this Account. See User Grants View ([GET /account/users/{username}/grants](/docs/api/account/#users-grants-view)) for details on how to configure grants for a restricted User.
     *
     * @return self
     */
     ...
     * Sets tfa_enabled
     *
     * @param bool|null $tfa_enabled A boolean value indicating if the User has Two Factor Authentication (TFA) enabled. See the Create Two Factor Secret ([POST /profile/tfa-enable](/docs/api/profile/#two-factor-secret-create)) endpoint to enable TFA.
     *
     * @return self
     */
