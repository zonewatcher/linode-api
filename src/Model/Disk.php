...
class Disk implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    const FILESYSTEM_RAW = 'raw';
    const FILESYSTEM_SWAP = 'swap';
    const FILESYSTEM_EXT3 = 'ext3';
    const FILESYSTEM_EXT4 = 'ext4';
    const FILESYSTEM_INITRD = 'initrd';
    ...
    public function getFilesystemAllowableValues()
    {
        return [
            self::FILESYSTEM_RAW,
            self::FILESYSTEM_SWAP,
            self::FILESYSTEM_EXT3,
            self::FILESYSTEM_EXT4,
            self::FILESYSTEM_INITRD,
        ];
    }
    ...
    public function listInvalidProperties()
    {
        ...
        if (!is_null($this->container['filesystem']) && !in_array($this->container['filesystem'], $allowedValues, true)) {
            $invalidProperties[] = sprintf(
                "invalid value '%s' for 'filesystem', must be one of '%s'",
     ...
     * Sets id
     *
     * @param int|null $id This Disk's ID which must be provided for all operations impacting this Disk.
     *
     * @return self
     */
     ...
     * Sets label
     *
     * @param string|null $label The Disk's label is for display purposes only.
     *
     * @return self
     */
    public function setLabel($label)
    {
        if (!is_null($label) && (mb_strlen($label) > 48)) {
            throw new \InvalidArgumentException('invalid length for $label when calling Disk., must be smaller than or equal to 48.');
        }
        if (!is_null($label) && (mb_strlen($label) < 1)) {
            throw new \InvalidArgumentException('invalid length for $label when calling Disk., must be bigger than or equal to 1.');
        }
     ...
     * Sets status
     *
     * @param string|null $status A brief description of this Disk's current state. This field may change without direct action from you, as a result of operations performed to the Disk or the Linode containing the Disk.
     *
     * @return self
     */
     ...
     * Sets size
     *
     * @param int|null $size The size of the Disk in MB.
     *
     * @return self
     */
     ...
     * Sets filesystem
     *
     * @param string|null $filesystem The Disk filesystem can be one of:    * raw - No filesystem, just a raw binary stream.   * swap - Linux swap area.   * ext3 - The ext3 journaling filesystem for Linux.   * ext4 - The ext4 journaling filesystem for Linux.   * initrd - initrd (uncompressed initrd, ext2, max 32 MB).
     *
     * @return self
     */
    ...
    {
        $allowedValues = $this->getFilesystemAllowableValues();
        if (!is_null($filesystem) && !in_array($filesystem, $allowedValues, true)) {
            throw new \InvalidArgumentException(
                sprintf(
                    "Invalid value '%s' for 'filesystem', must be one of '%s'",
                    $filesystem,
     ...
     * Sets created
     *
     * @param \DateTime|null $created When this Disk was created.
     *
     * @return self
     */
     ...
     * Sets updated
     *
     * @param \DateTime|null $updated When this Disk was last updated.
     *
     * @return self
     */
