...
/**
 * LinodeConfig Class Doc Comment
 *
 * @category Class
 * @package  ZoneWatcher\LinodeApiV4
 * @author   OpenAPI Generator team
 ...
 */
class LinodeConfig implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    /**
      ...
      * @var string
      */
    protected static $openAPIModelName = 'LinodeConfig';
    ...
    protected static $openAPITypes = [
        ...
        'run_level' => 'string',
        'virt_mode' => 'string',
        'interfaces' => '\ZoneWatcher\LinodeApiV4\Model\LinodeConfigInterface[]',
        'helpers' => '\ZoneWatcher\LinodeApiV4\Model\LinodeConfigHelpers',
        'label' => 'string',
        'devices' => '\ZoneWatcher\LinodeApiV4\Model\Devices',
        'root_device' => 'string'
    ];
    ...
    protected static $setters = [
        ...
        'virt_mode' => 'setVirtMode',
        'interfaces' => 'setInterfaces',
        'helpers' => 'setHelpers',
        'label' => 'setLabel',
        'devices' => 'setDevices',
        'root_device' => 'setRootDevice'
    ];
    ...
    protected static $getters = [
        ...
        'virt_mode' => 'getVirtMode',
        'interfaces' => 'getInterfaces',
        'helpers' => 'getHelpers',
        'label' => 'getLabel',
        'devices' => 'getDevices',
        'root_device' => 'getRootDevice'
    ];
    ...
    const RUN_LEVEL__DEFAULT = 'default';
    const RUN_LEVEL_SINGLE = 'single';
    const RUN_LEVEL_BINBASH = 'binbash';
    const VIRT_MODE_PARAVIRT = 'paravirt';
    const VIRT_MODE_FULLVIRT = 'fullvirt';
     ...
     * @return string[]
     */
    public function getRunLevelAllowableValues()
    {
        return [
            self::RUN_LEVEL__DEFAULT,
            self::RUN_LEVEL_SINGLE,
            self::RUN_LEVEL_BINBASH,
        ];
    }
     ...
     * @return string[]
     */
    public function getVirtModeAllowableValues()
    {
        return [
            self::VIRT_MODE_PARAVIRT,
            self::VIRT_MODE_FULLVIRT,
        ];
    }
    ...
    public function __construct(array $data = null)
    {
        ...
        $this->container['kernel'] = $data['kernel'] ?? null;
        $this->container['comments'] = $data['comments'] ?? null;
        $this->container['memory_limit'] = $data['memory_limit'] ?? null;
        $this->container['run_level'] = $data['run_level'] ?? null;
        $this->container['virt_mode'] = $data['virt_mode'] ?? null;
        $this->container['interfaces'] = $data['interfaces'] ?? null;
        $this->container['helpers'] = $data['helpers'] ?? null;
        $this->container['label'] = $data['label'] ?? null;
        $this->container['devices'] = $data['devices'] ?? null;
        $this->container['root_device'] = $data['root_device'] ?? null;
    }
    ...
    public function listInvalidProperties()
    {
        ...
        $allowedValues = $this->getRunLevelAllowableValues();
        if (!is_null($this->container['run_level']) && !in_array($this->container['run_level'], $allowedValues, true)) {
            $invalidProperties[] = sprintf(
                "invalid value '%s' for 'run_level', must be one of '%s'",
                $this->container['run_level'],
                implode("', '", $allowedValues)
            );
        }

        $allowedValues = $this->getVirtModeAllowableValues();
        if (!is_null($this->container['virt_mode']) && !in_array($this->container['virt_mode'], $allowedValues, true)) {
            $invalidProperties[] = sprintf(
                "invalid value '%s' for 'virt_mode', must be one of '%s'",
                $this->container['virt_mode'],
                implode("', '", $allowedValues)
            );
        }

        if (!is_null($this->container['label']) && (mb_strlen($this->container['label']) > 48)) {
        ...
        if (!is_null($this->container['root_device']) && !preg_match("/a-z, A-Z, 0-9, \/, _, -/", $this->container['root_device'])) {
            $invalidProperties[] = "invalid value for 'root_device', must be conform to the pattern /a-z, A-Z, 0-9, \/, _, -/.";
        }
     ...
     * Sets id
     *
     * @param int|null $id The ID of this Config.
     *
     * @return self
     */
     ...
     * Sets kernel
     *
     * @param string|null $kernel A Kernel ID to boot a Linode with. Defaults to \"linode/latest-64bit\".
     *
     * @return self
     */
     ...
     * Sets comments
     *
     * @param string|null $comments Optional field for arbitrary User comments on this Config.
     *
     * @return self
     */
     ...
     * Sets memory_limit
     *
     * @param int|null $memory_limit Defaults to the total RAM of the Linode.
     *
     * @return self
     */
     ...
     * Sets run_level
     *
     * @param string|null $run_level Defines the state of your Linode after booting. Defaults to `default`.
     *
     * @return self
     */
    ...
    {
        $allowedValues = $this->getRunLevelAllowableValues();
        if (!is_null($run_level) && !in_array($run_level, $allowedValues, true)) {
     ...
     * @return string|null
     */
    public function getVirtMode()
    {
        return $this->container['virt_mode'];
    }

    /**
     * Sets virt_mode
     *
     * @param string|null $virt_mode Controls the virtualization mode. Defaults to `paravirt`. * `paravirt` is suitable for most cases. Linodes running in paravirt mode   share some qualities with the host, ultimately making it run faster since   there is less transition between it and the host. * `fullvirt` affords more customization, but is slower because 100% of the VM   is virtualized.
     *
     * @return self
     */
    public function setVirtMode($virt_mode)
    {
        $allowedValues = $this->getVirtModeAllowableValues();
        if (!is_null($virt_mode) && !in_array($virt_mode, $allowedValues, true)) {
            throw new \InvalidArgumentException(
                sprintf(
                    "Invalid value '%s' for 'virt_mode', must be one of '%s'",
                    $virt_mode,
        ...
        $this->container['virt_mode'] = $virt_mode;
     ...
     * Gets interfaces
     *
     * @return \ZoneWatcher\LinodeApiV4\Model\LinodeConfigInterface[]|null
     */
     ...
     * Gets helpers
     *
     * @return \ZoneWatcher\LinodeApiV4\Model\LinodeConfigHelpers|null
     */
     ...
     * Sets helpers
     *
     * @param \ZoneWatcher\LinodeApiV4\Model\LinodeConfigHelpers|null $helpers helpers
     *
     * @return self
     */
     ...
     * Sets label
     *
     * @param string|null $label The Config's label is for display purposes only.
     *
     * @return self
     */
    public function setLabel($label)
    {
        if (!is_null($label) && (mb_strlen($label) > 48)) {
            throw new \InvalidArgumentException('invalid length for $label when calling LinodeConfig., must be smaller than or equal to 48.');
        }
        if (!is_null($label) && (mb_strlen($label) < 1)) {
            throw new \InvalidArgumentException('invalid length for $label when calling LinodeConfig., must be bigger than or equal to 1.');
        }
     ...
     * Gets devices
     *
     * @return \ZoneWatcher\LinodeApiV4\Model\Devices|null
     */
     ...
     * Sets devices
     *
     * @param \ZoneWatcher\LinodeApiV4\Model\Devices|null $devices devices
     *
     * @return self
     */
    ...
    {
        return $this->container['root_device'];
    }

    /**
     * Sets root_device
     *
     * @param string|null $root_device The root device to boot. * If no value or an invalid value is provided, root device will default to `/dev/sda`. * If the device specified at the root device location is not mounted, the Linode will not boot until a device is mounted.
     *
     * @return self
     */
    public function setRootDevice($root_device)
    {

        if (!is_null($root_device) && (!preg_match("/a-z, A-Z, 0-9, \/, _, -/", $root_device))) {
            throw new \InvalidArgumentException("invalid value for $root_device when calling LinodeConfig., must conform to the pattern /a-z, A-Z, 0-9, \/, _, -/.");
        }

        $this->container['root_device'] = $root_device;
